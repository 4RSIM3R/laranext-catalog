...
export default function PartnerIndex() {
    ...
    const load = useCallback(
        async (params: Record<string, any>) => {
            const response = await axios.get<Base<Partner[]>>(
                partner.fetch().url,
                {
                    params: {
                        ...params,
    ...
    const onDelete = (e: { preventDefault: () => void }) => {
        e.preventDefault();
        destroy(partner.destroy(id).url, FormResponse);
    };
            ...
            cell: (row) => (
                <DropdownMenu>
                    ...
                    <DropdownMenuContent align="center">
                        <Link
                            href={partner.show(row.row.original.id).url}
                            method="get"
                        >
                            <DropdownMenuItem>
                                <Eye /> Detail
            ...
            <div className="flex flex-row items-center justify-between">
                <div className="flex flex-col">
                    ...
                    <p className="text-sm text-gray-500">
                        Manage your partners and their visibility
                    </p>
                </div>
                <Link href={partner.create().url}>
                    <Button>
...
PartnerIndex.layout = (page: React.ReactNode) => <AppLayout>{page}</AppLayout>;
