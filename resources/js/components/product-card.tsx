...
export function ProductCard({ props }: Props) {
    const handleWhatsAppClick = (e: React.MouseEvent) => {
        e.preventDefault();
        e.stopPropagation();
        const phoneNumber = props.phone_number?.replace(/[^0-9]/g, '') || '';
        if (phoneNumber) {
            const message = `Halo, saya tertarik dengan produk ${props.title}. Apakah masih tersedia?`;
            ...
            <Link href={product.show(props.slug).url}>
                <div className="relative aspect-[4/3] overflow-hidden bg-gray-100 dark:bg-gray-800">
                    ...
                    <button
                        ...
                        className="absolute top-3 right-3 flex h-12 w-12 items-center justify-center rounded-full bg-green-500 shadow-lg transition-all hover:scale-110 hover:bg-green-600"
